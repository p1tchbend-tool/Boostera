using System;
using System.Collections.Generic;
using System.Windows.Forms;

namespace Boostera
{
    internal class HotKey : Form
    {
        internal static readonly int MOD_KEY_NONE = 0;
        internal static readonly int MOD_KEY_ALT = 1;
        internal static readonly int MOD_KEY_CONTROL = 2;
        internal static readonly int MOD_KEY_SHIFT = 4;
        internal event EventHandler OnHotKey;

        private readonly int WM_HOTKEY = 0x0312;
        private List<int> ids = new List<int>();

        protected override void WndProc(ref Message m)
        {
            base.WndProc(ref m);
            if (m.Msg == WM_HOTKEY) OnHotKey(this, new HotKeyEventArgs((int)m.WParam));
        }

        internal bool Add(int modKey, Keys key, int id)
        {
            if (NativeMethods.RegisterHotKey(this.Handle, id, modKey, key) != 0)
            {
                if (!ids.Contains(id)) ids.Add(id);
                return true;
            }
            return false;
        }

        internal bool Remove(int id)
        {
            if (NativeMethods.UnregisterHotKey(this.Handle, id) != 0)
            {
                if (ids.Contains(id)) ids.Remove(id);
                return true;
            }
            return false;
        }

        internal void RemoveAll()
        {
            ids.ForEach(x =>
            {
                NativeMethods.UnregisterHotKey(this.Handle, x);
            });
            ids = new List<int>();
        }

        protected override void Dispose(bool disposing)
        {
            ids.ForEach(x =>
            {
                NativeMethods.UnregisterHotKey(this.Handle, x);
            });
            base.Dispose(disposing);
        }

        internal class HotKeyEventArgs : EventArgs
        {
            public int Id;
            public HotKeyEventArgs(int id)
            {
                Id = id;
            }
        }

        private void HotKey_Load(object sender, EventArgs e)
        {

        }

        private void InitializeComponent()
        {
            this.SuspendLayout();
            // 
            // HotKey
            // 
            this.ClientSize = new System.Drawing.Size(284, 261);
            this.Name = "HotKey";
            this.Load += new EventHandler(this.HotKey_Load);
            this.ResumeLayout(false);

        }

        public HotKey()
        {
            InitializeComponent();
            this.Hide();
        }
    }
}
